...
theorem _root_.FWRust.Core.Term.Diverges.no_value {value : Value} :
...
theorem _root_.FWRust.Core.Term.Diverges.step {store store' : ProgramStore} {lifetime : Lifetime}
    {term term' : Term} :
    Term.Diverges term →
    Step store lifetime term store' term' →
...
theorem _root_.FWRust.Core.Term.Diverges.multiStep {store finalStore : ProgramStore}
