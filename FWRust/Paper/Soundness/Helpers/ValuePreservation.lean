  ...
  | copy hLv hcopy _hreadProhibited =>
      ...
      | copy hread =>
          ...
          have hnonOwner : valueOwnedLocation? value = none :=
            copy_value_nonOwner_whenInitialized hcopy hvalidValue
          exact ⟨hvalidStore,
...
theorem validRuntimeState_copy_step_of_safe {store : ProgramStore} {env env₂ : Env}
  ...
  have htermHeap : TermOwnerTargetsHeap (.val value) := by
    ...
    | copy hLv hcopy hreadProhibited =>
        have hnonOwner : valueOwnedLocation? value = none := by
          cases hstep with
          | copy hread =>
              rcases readPreservation_of_safe_whenInitialized hsafe hLv with
                ⟨readValue, runtimeSlot, hreadPreserved, hslotValue, hvalidValue⟩
              ...
              cases hslotEq
              cases hslotValue
              exact copy_value_nonOwner_whenInitialized hcopy hvalidValue
...
theorem storePreservation_copy_step_of_safe {store : ProgramStore} {env env₂ : Env}
    ...
    TermTyping env typing lifetime (.copy lv) ty env₂ →
    Step store lifetime (.copy lv) store (.val value) →
    store ∼ env₂ ∧ ValidValue store value ty := by
  intro hsafe htyping hstep
  cases htyping with
  | copy hLv hcopy hreadProhibited =>
      exact ⟨hsafe,
...
theorem preservation_move_var_step_runtime_whenInitialized_of_preserved
  ...
  intro hsafe hvalidRuntime henvSlot hmove _htyping hstep hvalidValue
    hpreserveOld
  exact ⟨validRuntimeState_move_step hvalidRuntime hstep,
    storePreservation_move_var_step
...
theorem preservation_box_redex_runtime_of_validValue {store store' : ProgramStore}
    ...
    ValidValue store value ty →
    Step store lifetime (.box (.val value)) store' (.val (.ref ref)) →
    ValidRuntimeState store' (.val (.ref ref)) ∧ store' ∼ env ∧
  ...
  | box hfresh hbox =>
      ...
      exact ⟨validRuntimeState_box_step_of_validValue hvalidRuntime hoperandValid
          ...
          fullSafeAbstraction_boxAt hfresh hsafe,
...
theorem preservation_declare_redex_runtime_of_validValue {store store' : ProgramStore}
    ...
    ValidRuntimeState store' (.val .unit) ∧
      store' ∼ env.update x { ty := .ty ty, lifetime := lifetime } ∧
  ...
  have hfreshStore : store.fresh (VariableProjection x) :=
    fullSafeAbstraction_store_fresh_var hsafe hfresh
  cases hstep with
  | declare hstore' =>
      subst hstore'
      have hsafe' :
        ...
        exact fullSafeAbstraction_declare hsafe hfresh
...
theorem preservation_assign_var_old_nonOwner_step_runtime_whenInitialized_of_preserved
  ...
  exact ⟨validRuntimeState_assign_step_old_nonOwner_whenInitialized
      ...
      hdrops,
    storePreservation_assign_var_old_nonOwner_of_preserved
...
theorem preservation_assign_var_envShape_step_runtime_of_frames
  ...
  have hnonOwner : PartialValueNonOwner oldSlot.value :=
    fullSafeAbstraction_var_read_nonOwner_of_envShape
      hsafe henvX hread hshape
...
theorem preservation_blockB_value_step_runtime_no_slots
  ...
  have hsafe' : store' ∼ env.dropLifetime blockLifetime := by
    cases hstep with
    | blockB hdrops =>
        exact fullSafeAbstraction_dropsLifetime_no_slots hsafe hnoLifetime hdrops
...
theorem preservation_copy_multistep_runtime_of_safe {store finalStore : ProgramStore}
    ...
    ValidRuntimeState finalStore (.val finalValue) ∧ finalStore ∼ env₂ ∧
...
theorem preservation_copy_multistep_runtime_whenInitialized
    ...
    ValidRuntimeState finalStore (.val finalValue) ∧
      FullSafeAbstraction finalStore env₂ ∧
...
theorem preservation_box_context_multistep_runtime
    ...
    (ValidRuntimeState store term →
      ...
      ValidRuntimeState midStore (.val value) ∧ midStore ∼ env₂ ∧
...
theorem preservation_box_context_terminal_multistep_runtime_whenInitialized
    ...
    (∀ {midStore value},
      ...
      ValidRuntimeState midStore (.val value) ∧
        FullSafeAbstraction midStore env₂ ∧
