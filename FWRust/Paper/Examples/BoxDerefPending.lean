...
theorem blockBoxInt_noStale :
    EnvWriteNoStaleBorrowTargets 0 blockBoxIntEnv (.deref (.var "p")) .int
      blockBoxIntEnv := by
  intro _written _x _slot _mutable _targets _target heffective _hslot
    hcontains _htarget _hmay
  exact blockBoxInt_no_contains_borrow hcontains
