  ...
  · intro name slot hslot
    ...
    · by_cases hp : name = "p"
      ...
      · have hnone : dropMoveStartEnv.slotAt name = none := by
        ...
        cases hslot
  · intro _source _inner _lifetime _hsource ihSource
    rcases ihSource with hsourceShape | hsourceShape | hsourceShape | hsourceShape
...
theorem dropMoveStartEnv_borrow_lval_shape {lv : LVal} {mutable : Bool}
    {targets : List LVal} {lifetime : Lifetime} :
    LValTyping dropMoveStartEnv lv (.ty (.borrow mutable targets)) lifetime →
      lv = .deref (.var "p") ∧ mutable = true ∧ targets = [.var "x"] ∧
        lifetime = Lifetime.root := by
  intro htyping
  rcases dropMoveStartEnv_lvalTyping_shape htyping with hshape | hshape | hshape | hshape
  · rcases hshape with ⟨_hlv, hty, _hlifetime⟩
    cases hty
  · rcases hshape with ⟨_hlv, hty, _hlifetime⟩
    cases hty
  · rcases hshape with ⟨hlv, hty, hlifetime⟩
    cases hty
...
theorem dropMoveP_freshUpdate :
  ...
  · intro lv mutable targets borrowLifetime _hbase htyping
    ...
    rcases dropMoveStartEnv_borrow_lval_shape hborrow with
      ⟨hlv, _hmutable, _htargets, _hlifetime⟩
    subst hlv
    exact False.elim (_hbase rfl)
  · intro lv mutable targets borrowLifetime _hbase htyping
    have hborrow :
        LValTyping dropMoveStartEnv lv (.ty (.borrow mutable targets))
          borrowLifetime := by
      simpa [dropMoveStartEnv, staleJoinPSlot] using htyping
    rcases dropMoveStartEnv_borrow_lval_shape hborrow with
      ⟨hlv, hmutable, htargets, _hlifetime⟩
    subst hlv
    subst hmutable
    subst htargets
    refine ⟨.int, Lifetime.root, ?_⟩
    exact LValTargetsTyping.singleton
      (@LValTyping.var dropMoveStartEnv "x" staleJoinXSlot (by
    ...
    (by
      ...
      · subst hx
        have hslotEq : slot = staleJoinXMovedSlot := by
          exact (Option.some.inj
            (by simpa [dropMoveJoinEnv, dropMovePMovedEnv, dropMoveStartEnv,
              dropMoveXEnv, staleJoinXMovedSlot, staleJoinPMovedSlot,
              staleJoinPSlot, staleJoinXSlot, Env.update] using hslot)).symm
        subst hslotEq
        exact Or.inl ⟨rfl, rfl, rfl⟩
      · by_cases hp : name = "p"
        · subst hp
          have hslotEq : slot = staleJoinPMovedSlot := by
            exact (Option.some.inj
              (by simpa [dropMoveJoinEnv, dropMovePMovedEnv,
...
theorem boxedLive_borrowUnion_y_x :
  ...
  · intro candidate hcandidate
    ...
    · exact PartialTyStrengthens.borrow (by
        intro target htarget
        exact List.mem_append.mpr (Or.inl htarget)) (by
        intro _ hnil
        simp at hnil)
    · exact PartialTyStrengthens.borrow (by
        intro target htarget
        change target ∈ [LVal.var "y"] ++ [LVal.var "x"]
        exact List.mem_append.mpr (Or.inr htarget)) (by
        intro _ hnil
        simp at hnil)
  · intro upper hupper
...
theorem boxedLive_assign_effective_write_written {result : Env}
    {written : LVal} :
    EnvWriteEffectiveWrite 0 boxedLiveStartEnv (.var "p")
      (.box (.borrow true [.var "x"])) result written →
    written = .var "p" := by
  intro hwrite
  cases hwrite with
...
theorem boxedLive_assign_noStale :
    EnvWriteNoStaleBorrowTargets 0 boxedLiveStartEnv (.var "p")
      (.box (.borrow true [.var "x"])) boxedLivePAssignedEnv := by
  intro written x slot mutable targets target hwrite hslot hcontains htarget
    hmayRead
  have hwritten := boxedLive_assign_effective_write_written hwrite
  subst written
  ...
  by_cases hp : x = "p"
  · subst hp
    have hslotTy : slot.ty = .ty (.box (.borrow true [.var "x"])) := by
      simpa [boxedLivePAssignedEnv, boxedLivePXSlot, Env.update] using
        (congrArg (fun slotOpt => Option.map EnvSlot.ty slotOpt) hslot).symm
  ...
  · by_cases hy : x = "y"
    · subst hy
      have hslotTy : slot.ty = .ty .int := by
        simpa [boxedLivePAssignedEnv, boxedLiveStartEnv, boxedLiveXYEnv,
          staleJoinYSlot, boxedLivePYSlot, boxedLivePXSlot, Env.update] using
          (congrArg (fun slotOpt => Option.map EnvSlot.ty slotOpt) hslot).symm
      rw [hslotTy] at hcontainsTy
      cases hcontainsTy
    · by_cases hx : x = "x"
      · subst hx
        have hslotTy : slot.ty = .ty .int := by
          simpa [boxedLivePAssignedEnv, boxedLiveStartEnv, boxedLiveXYEnv,
            dropMoveXEnv, staleJoinXSlot, staleJoinYSlot, boxedLivePYSlot,
            boxedLivePXSlot, Env.update] using
            (congrArg (fun slotOpt => Option.map EnvSlot.ty slotOpt) hslot).symm
        rw [hslotTy] at hcontainsTy
        cases hcontainsTy
      · have hnone : boxedLivePAssignedEnv.slotAt x = none := by
          simp [boxedLivePAssignedEnv, boxedLiveStartEnv, boxedLiveXYEnv,
            dropMoveXEnv, Env.update, Env.empty, hp, hy, hx]
        rw [hslot] at hnone
        cases hnone
...
theorem boxedLiveFalseBranch_typing :
  ...
  · exact TermListTyping.singleton (TermTyping.assign
        ...
        boxedLive_assign_noStale
        boxedLive_assign_ranked
        (Coherent.whenInitialized boxedLiveAssigned_coherent)
