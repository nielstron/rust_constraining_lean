...
theorem PartialTyStrengthens.to_eraseName_of_fresh {left right : PartialTy}
  ...
  | @borrow mutable leftTargets rightTargets hsubset hnonempty =>
      intro hfresh
      rw [PartialTy.eraseName, Ty.eraseName]
      exact PartialTyStrengthens.borrow
        (by
          intro target hmem
          have htargetRight : target ∈ rightTargets := hsubset hmem
          have hbaseNe : LVal.base target ≠ ghost := by
            intro hbase
            exact hfresh (by
              simp [PartialTy.allVars, Ty.allVars, List.mem_map]
              exact ⟨target, hmem, hbase⟩)
          exact List.mem_filter.mpr
            ⟨htargetRight, decide_eq_true hbaseNe⟩)
        (by
          intro hfilteredNonempty hleftEmpty
          exact hnonempty
            (by
              intro hrightEmpty
              subst hrightEmpty
              simp at hfilteredNonempty)
            hleftEmpty)
  | undefLeft _hinner ih =>
      intro hfresh
      exact PartialTyStrengthens.undefLeft
...
theorem CoherentWhenInitialized.erase_ghost {env : Env} {ghost : Name} :
    CoherentWhenInitialized env →
    Env.TypeNameFresh (env.erase ghost) ghost →
    CoherentWhenInitialized (env.erase ghost) := by
  intro hcoherent hfresh lv mutable targets borrowLifetime htyping hinitialized
  have htypingEnv :
      LValTyping env lv (.ty (.borrow mutable targets)) borrowLifetime :=
    LValTyping.erase_to_env.1 htyping
  have hinitializedEnv : BorrowTargetsInitialized env targets := by
    intro target htarget
    rcases hinitialized target htarget with
      ⟨targetTy, targetLifetime, htargetTyping⟩
    exact ⟨targetTy, targetLifetime,
      LValTyping.erase_to_env.1 htargetTyping⟩
  rcases hcoherent lv mutable targets borrowLifetime htypingEnv
      hinitializedEnv with
    ⟨targetTy, targetLifetime, htargets⟩
  have hborrowFresh :
      ghost ∉ PartialTy.allVars (.ty (.borrow mutable targets)) :=
    LValTyping.typeNameFresh.1 htyping hfresh
  ...
  exact TermTyping.rec
    ...
    (by
      ...
      exact ⟨TermTyping.assign hRhsErased hLhsErased
        ...
        (CoherentWhenInitialized.erase_ghost hcoh hfreshWrite)
