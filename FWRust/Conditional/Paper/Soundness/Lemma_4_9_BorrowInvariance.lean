...
theorem TermTyping.retype_of_sourceTerm {env₁ env₂ : Env}
  ...
  exact TermTyping.rec
    ...
    (fun _hcondition _htrue _hfalse hjoin henvJoin
    ...
    (fun _hcondition _htrue _hfalse hdiverges ihCondition ihTrue ihFalse
      ...
      TermTyping.iteTrueDiverging
        ...
        (ihFalse (SourceTerm.ite_falseBranch hsource))
        hdiverges)
    (fun hchild _hcondition _hbody hdiverges ihCondition ihBody hsource =>
      TermTyping.whileLoopDiverging hchild
        (ihCondition (SourceTerm.while_condition hsource))
        (ihBody (SourceTerm.while_body hsource)) hdiverges)
    (fun hchild hjoin hcontained hnameFresh _hcondition _hbody hdrop
        ihCondition ihBody hsource =>
      TermTyping.whileLoop hchild hjoin hcontained hnameFresh
        (ihCondition (SourceTerm.while_condition hsource))
        (ihBody (SourceTerm.while_body hsource)) hdrop)
    (fun _hterm ih hsource =>
...
theorem BorrowTargetsInitialized.of_update_of_not_pathConflicts {env : Env}
    {x : Name} {oldSlot slot : EnvSlot} {targets : List LVal} :
    env.slotAt x = some oldSlot →
    ¬ WriteProhibited env (.var x) →
    BorrowTargetsInitialized (env.update x slot) targets →
    (∀ target, target ∈ targets → ¬ target ⋈ (.var x)) →
    BorrowTargetsInitialized env targets := by
  intro henvX hnotWrite hinitialized hnotTargets target htarget
  rcases hinitialized target htarget with
    ⟨targetTy, targetLifetime, htargetTyping⟩
  have hrestore : (env.update x slot).update x oldSlot = env := by
    obtain ⟨g⟩ := env
    simp only [Env.update]
    congr 1
    funext y
    by_cases hy : y = x
    · subst hy
      simpa using henvX.symm
    · simp [hy]
  have hnotWriteRestore :
      ¬ WriteProhibited ((env.update x slot).update x oldSlot) (.var x) := by
    rwa [hrestore]
  have htargetTypingRestore :
      LValTyping ((env.update x slot).update x oldSlot) target
        (.ty targetTy) targetLifetime :=
    (LValTyping.update_of_not_pathConflicts (env := env.update x slot)
      (x := x) (slot := oldSlot) hnotWriteRestore).1 htargetTyping
      (hnotTargets target htarget)
  exact ⟨targetTy, targetLifetime, by
    simpa [hrestore] using htargetTypingRestore⟩

theorem ValidPartialValueWhenInitialized.update_env_of_not_pathConflicts
    {env : Env} {store : ProgramStore} {x : Name} {oldSlot slot : EnvSlot}
    {value : PartialValue} {partialTy : PartialTy} :
    env.slotAt x = some oldSlot →
    ¬ WriteProhibited env (.var x) →
    ¬ WriteProhibited (env.update x slot) (.var x) →
    ...
    ValidPartialValueWhenInitialized env store value partialTy →
    ValidPartialValueWhenInitialized (env.update x slot) store value partialTy := by
  intro henvX hnotWriteSource hnotWriteUpdated hvarsFresh hvalid
  induction hvalid with
  | unit =>
      exact ValidPartialValueWhenInitialized.unit
  | int =>
      ...
      exact ValidPartialValueWhenInitialized.borrowStale (by
        ...
        exact hstale
          (BorrowTargetsInitialized.of_update_of_not_pathConflicts
            henvX hnotWriteSource hinitialized hnotTargets))
  | @box location boxSlot inner hslot _hinner ih =>
      exact ValidPartialValueWhenInitialized.box hslot
        (ih (by
          intro hmem
...
theorem ty_eqv_imp_strengthens : ∀ {a b : Ty},
  ...
  | .borrow m ta, b, h => by
      cases b with
      | borrow m' tb =>
          obtain ⟨rfl, hsub, hsubBack⟩ := h
          exact PartialTyStrengthens.borrow hsub (by
            intro htb hta
            apply htb
            cases tb with
            | nil => rfl
            | cons head tail =>
                have hheadTb : head ∈ head :: tail := by simp
                have hheadMem : head ∈ ta := hsubBack hheadTb
                rw [hta] at hheadMem
                cases hheadMem)
...
/-- Lvalue typings transport backwards through an environment strengthening.

The borrow case is structural as well.  The strengthening witness exposes a
nonempty source target list contained in the result target list.  The mutual
induction hypothesis transports every member of the result-side target
derivation, and `of_nonempty_members_bounded` reassembles precisely that source
list.  Thus no global lvalue ranking is needed. -/
theorem lvalTyping_back_of_envStrengthens {source result : Env}
      ...
      LValTyping result lv pty lifetime →
        ∃ sourcePty sourceLifetime,
          LValTyping source lv sourcePty sourceLifetime ∧
            PartialTyStrengthens sourcePty pty ∧
            sourceLifetime ≤ lifetime := by
  intro lv pty lifetime htyping
  refine LValTyping.rec
    (motive_1 := fun lv pty lifetime _ =>
      ∃ sourcePty sourceLifetime,
        LValTyping source lv sourcePty sourceLifetime ∧
          PartialTyStrengthens sourcePty pty ∧
          sourceLifetime ≤ lifetime)
    (motive_2 := fun targets pty lifetime _ =>
  ...
  case var =>
    ...
    | some sourceSlot =>
        ...
        exact ⟨sourceSlot.ty, sourceSlot.lifetime,
          ...
          by rw [hx.1]; exact LifetimeOutlives.refl _⟩
  case box =>
    intro lv inner lifetime _hinner ih
    rcases ih with
  ...
  case boxFull =>
    ...
    exact ⟨.ty sourceInner, sourceLifetime,
      ...
      hsourceLifetime⟩
  case borrow =>
    intro lv mutable resultTargets borrowLifetime targetLifetime targetTy
      _hborrow htargets ihBorrow ihTargets
    rcases ihBorrow with
      ⟨sourcePty, sourceBorrowLifetime, hsourceBorrowRaw,
        hsourceBorrowStrength, _hsourceBorrowLifetime⟩
    rcases PartialTyStrengthens.to_borrow_right_nonempty
        hsourceBorrowStrength with
      ⟨sourceTargets, hsourcePtyEq, hsubset, hsourceNonempty⟩
    subst hsourcePtyEq
    have hresultTargetsNonempty : resultTargets ≠ [] := by
      intro hempty
      subst hempty
      cases htargets
    have hsourceTargetsNonempty : sourceTargets ≠ [] :=
      hsourceNonempty hresultTargetsNonempty
    rcases LValTargetsTyping.output_full htargets with
      ⟨upperTy, htargetTy⟩
    subst htargetTy
    rcases LValTargetsTyping.of_nonempty_members_bounded
        hsourceTargetsNonempty
        (by
          intro target htarget
          exact ihTargets target (hsubset htarget)) with
      ⟨sourceTargetTy, sourceTargetLifetime, hsourceTargets,
        hsourceTargetStrength, hsourceTargetLifetime⟩
    exact ⟨.ty sourceTargetTy, sourceTargetLifetime,
      LValTyping.borrow hsourceBorrowRaw hsourceTargets,
      hsourceTargetStrength, hsourceTargetLifetime⟩
  case singleton =>
    intro target ty lifetime _htarget ihTarget selected hselected
    simp only [List.mem_singleton] at hselected
    subst hselected
    rcases ihTarget with
      ⟨sourcePty, sourceLifetime, hsourceTarget, hsourceStrength,
        hsourceLifetime⟩
    rcases PartialTyStrengthens.to_ty_right hsourceStrength with
      ⟨sourceTy, hsourcePtyEq⟩
    subst hsourcePtyEq
    exact ⟨sourceTy, sourceLifetime, hsourceTarget, hsourceStrength,
      hsourceLifetime⟩
  case cons =>
    intro target rest headTy headLifetime restLifetime lifetime restTy unionTy
      _hhead _hrest hunion hintersection ihHead ihRest selected hselected
    ...
    · subst hselected
      ...
      rcases PartialTyStrengthens.to_ty_right hsourceStrength with
        ⟨sourceTy, hsourcePtyEq⟩
      subst hsourcePtyEq
      exact ⟨sourceTy, sourceLifetime, hsourceTarget,
        partialTyStrengthens_trans hsourceStrength
          (PartialTyUnion.left_strengthens hunion),
        LifetimeOutlives.trans hsourceLifetime
          (LifetimeIntersection.left_le hintersection)⟩
    · rcases ihRest selected hselected with
        ⟨sourceTy, sourceLifetime, hsourceTarget, hsourceStrength,
          hsourceLifetime⟩
      exact ⟨sourceTy, sourceLifetime, hsourceTarget,
        partialTyStrengthens_trans hsourceStrength
          (PartialTyUnion.right_strengthens hunion),
...
theorem lvalTyping_initialized_back_of_envStrengthens {source result : Env}
  ...
  intro htyping
  rcases lvalTyping_back_of_envStrengthens hstrength htyping with
    ⟨sourcePty, sourceLifetime, hsource, hstrengthTy, _hsourceLifetime⟩
...
/-!
The weak type invariant is recoverable at a conditional join.  In particular,
`T-If` does not need to carry full `WellFormedTy join joinTy lifetime` merely to
establish the invariant used by preservation.

The key point is that a target in the joined result type originates in one of
the branch result types.  Branch weak well-formedness keeps that target's base
slot live through the join.  If the full target happens to be initialized in
the joined environment, the joined environment's contained-borrow base
decomposition bounds its typing lifetime.  Thus no same-shape transport (and no
assumption that a moved-out branch target remains initialized) is required.
-/

/-- A full type is weakly well formed as soon as all borrows it contains satisfy
the weak target invariant. -/
theorem WellFormedTyWhenInitialized.of_tyBorrowsWellFormed
    {env : Env} {ty : Ty} {lifetime : Lifetime} :
    TyBorrowsWellFormedWhenInitialized env ty lifetime →
      WellFormedTyWhenInitialized env ty lifetime := by
  intro hborrows
  exact wellFormedTyWhenInitialized_of_containedBorrowTargets
    (fun _mutable _targets hcontains => hborrows hcontains)

/-- Weak well-formedness of a joined result type, derived from the two branch
result types and the environment/type joins.  Only the weak contained-borrow
invariant of each branch environment is needed. -/
theorem wellFormedTyWhenInitialized_join
    ...
    (hcbwfLeft : ContainedBorrowsWellFormedWhenInitialized left)
    (hcbwfRight : ContainedBorrowsWellFormedWhenInitialized right)
    (hwellLeft : WellFormedTyWhenInitialized left leftTy lifetime)
    (hwellRight : WellFormedTyWhenInitialized right rightTy lifetime) :
    WellFormedTyWhenInitialized join joinTy lifetime := by
  have hdecomp :=
    containedBorrowsWellFormedWhenInitialized_join_base_decomp
      henvJoin hcbwfLeft hcbwfRight
  apply WellFormedTyWhenInitialized.of_tyBorrowsWellFormed
  intro mutable targets hcontains target htarget
  rcases PartialTyUnion.contained_borrow_member htyJoin hcontains htarget with
    ⟨leftTargets, hcontainsLeft, htargetLeft⟩ |
    ⟨rightTargets, hcontainsRight, htargetRight⟩
  · have hleftTarget :=
      (borrowTargetsWellFormedInSlotWhenInitialized_of_wellFormedTy_contains
        hwellLeft hcontainsLeft) target htargetLeft
    rcases hleftTarget.1 with ⟨baseSlot, hbaseSlot, hbaseOutlives⟩
    rcases EnvStrengthens.slot_forward (EnvJoin.left_le henvJoin) hbaseSlot with
      ⟨joinBaseSlot, hjoinBaseSlot, hbaseLifetime, _hstrengthens⟩
    have hjoinBaseOutlives : joinBaseSlot.lifetime ≤ lifetime := by
      rw [← hbaseLifetime]
      exact hbaseOutlives
    have hbase : LValBaseOutlives join target lifetime :=
      ⟨joinBaseSlot, hjoinBaseSlot, hjoinBaseOutlives⟩
    refine ⟨hbase, ?_⟩
    rintro ⟨targetTy, targetLifetime, htargetTyping⟩
    have htargetBound :=
      (lvalTyping_lifetime_le_baseSlot hdecomp htargetTyping
        joinBaseSlot hjoinBaseSlot).1
    exact ⟨targetTy, targetLifetime, htargetTyping,
      LifetimeOutlives.trans htargetBound hjoinBaseOutlives, hbase⟩
  · have hrightTarget :=
      (borrowTargetsWellFormedInSlotWhenInitialized_of_wellFormedTy_contains
        hwellRight hcontainsRight) target htargetRight
    rcases hrightTarget.1 with ⟨baseSlot, hbaseSlot, hbaseOutlives⟩
    rcases EnvStrengthens.slot_forward (EnvJoin.right_le henvJoin) hbaseSlot with
      ⟨joinBaseSlot, hjoinBaseSlot, hbaseLifetime, _hstrengthens⟩
    have hjoinBaseOutlives : joinBaseSlot.lifetime ≤ lifetime := by
      rw [← hbaseLifetime]
      exact hbaseOutlives
...
theorem wellFormedWhenInitialized_iteJoin_of_obligations
    ...
    {leftTy rightTy joinTy : Ty} :
    EnvJoin left right join →
    PartialTyJoin (.ty leftTy) (.ty rightTy) (.ty joinTy) →
    WellFormedEnvWhenInitialized left lifetime →
    WellFormedEnvWhenInitialized right lifetime →
    WellFormedTyWhenInitialized left leftTy lifetime →
    WellFormedTyWhenInitialized right rightTy lifetime →
    WellFormedEnvWhenInitialized join lifetime ∧
      WellFormedTyWhenInitialized join joinTy lifetime := by
  intro hjoin htyJoin hleft hright hwellLeft hwellRight
  exact ⟨
    wellFormedEnvWhenInitialized_join_of_obligations
      hjoin hleft hright,
    wellFormedTyWhenInitialized_join htyJoin hjoin hleft.1 hright.1
      hwellLeft hwellRight⟩
...
theorem typingPreservesWellFormed_of_ruleCarriedObligations_core_bounded
    ...
    {typing : StoreTyping} {lifetime : FWRust.Conditional.Core.Lifetime}
    {term : FWRust.Conditional.Core.Term} {ty : FWRust.Conditional.Core.Ty} :
    term.size ≤ fuel →
  ...
  | succ fuel ihFuel =>
      ...
      refine TermTyping.rec
        ...
        (fun {_env₁ _env₂ _env₃ _typing _lifetime _targetLifetime _lhs
              _oldTy _rhs _rhsTy}
            hRhs _hLhsPost hshape hwellRhs hwrite _hnoStale _hranked _hwriteCoh
          ...
          by
            ...
            have hcbwf3 :=
              containedBorrowsWellFormedWhenInitialized_assign
                result.1.1 hcontained hwrite hnotWrite
            exact ⟨⟨hcbwf3,
        ...
        (fun {_env₁ _env₂ _env₃ _env₄ _env₅ _typing _lifetime _condition
          ...
          wellFormedWhenInitialized_iteJoin_of_obligations
            _henvJoin _hjoin trueResult.1 falseResult.1 trueResult.2 falseResult.2)
        ...
        (fun {_env₁ _env₂ _env₃ _typing _lifetime _bodyLifetime _condition
              _body _bodyTy}
            _hchild _hcondition _hbody _hdiverges ihCondition _ihBody hsize
            htypingEq hwellFormed =>
          let conditionResult := ihCondition
            (by simp [Term.size, Term.sizeList] at hsize ⊢; omega)
            htypingEq hwellFormed
          ⟨conditionResult.1, WellFormedTyWhenInitialized.unit⟩)
        (fun {_env₁ _envBack _envInv _env₂ _env₃ _typing _lifetime
              _bodyLifetime _condition _body _bodyTy}
            _hchild hjoin hcontained _hnameFresh _hcondition _hbody _hdrop
            ihCondition _ihBody hsize htypingEq hwellFormed =>
          let invWellFormed :
              WellFormedEnvWhenInitialized _envInv _lifetime :=
            ⟨hcontained,
              EnvSlotsOutlive.of_lifetimesPreserved hwellFormed.2
                (EnvJoin.lifetimesPreserved_left hjoin)⟩
          let conditionResult := ihCondition
            (by simp [Term.size, Term.sizeList] at hsize ⊢; omega)
...
theorem typingPreservesWellFormedWhenInitialized_of_ruleCarriedObligations_core_bounded
  ...
  | succ fuel ihFuel =>
      ...
      refine TermTyping.rec
        ...
        (fun {_env₁ _env₂ _env₃ _typing _lifetime _targetLifetime _lhs
              _oldTy _rhs _rhsTy}
            hRhs _hLhsPost _hshape _hwellRhs hwrite _hnoStale _hranked _hwriteCoh hcontained
...
theorem RuntimeFrame.value_reachesWhenInitialized_ne_var_of_varsProtectedIn
    ...
    RuntimeFrame.ReachesWhenInitialized sourceEnv store (.value value) (.ty ty)
      location →
    location ≠ VariableProjection x := by
  intro hwellFormed hsafe hheap hvalueHeap hwellTy hvars hnotWriteSource
...
theorem preservation_assign_var_step_runtime_whenInitialized_of_frames
  ...
  have hnewValidWrite :
    ...
    exact ValidPartialValueWhenInitialized.update_env_of_not_pathConflicts
      henvX hnotWriteSource hnotWriteUpdated hvarsTyFresh
      hnewValidWriteOldEnv
  have hslotXWrite :
      writtenStore.slotAt (VariableProjection x) =
        some { value := .value value, lifetime := envSlot.lifetime } := by
    rw [hwriteEq]
    ...
    · intro y otherEnvSlot hyx henvY
      ...
      have hvalidOldWrite :
        ...
        ValidPartialValueWhenInitialized.update_env_of_not_pathConflicts
          henvX hnotWriteSource hnotWriteUpdated hvarsOtherFresh
          hvalidOldWriteOldEnv
      exact ⟨oldValue, hslotYWrite, hvalidOldWrite⟩
  have hvalueHeap : ValueOwnerTargetsHeap value :=
    TermOwnerTargetsHeap.value
      (termOwnerTargetsHeap_assign_inner
...
theorem preservation_assign_var_step_runtime_whenInitialized_of_wellFormed
  ...
  intro hwellFormed hlinear hsafe hvalidRuntime hLhs _hshape hwellTy hwrite hnotWrite
  ...
  | assign hread hwriteStoreWritten hdrops =>
      ...
      have hnotWriteSource : ¬ WriteProhibited env (.var x) := by
        exact not_writeProhibited_var_of_update_self hlinear (by
      ...
      exact preservation_assign_var_step_runtime_whenInitialized_of_frames
        (lifetime := lifetime)
        hwellOut hsafe hvalidRuntime henvSlot hwrite hnotWriteSource
        hnotWrite hvalidValue hread hwriteStoreWritten hdrops
...
theorem safeAbstractionWhenInitialized_update_owner_spine_of_frames
  ...
  have hrootValidFinalUpdated :
      ValidPartialValueWhenInitialized
        ...
        rootSlot.value updatedTy :=
    ValidPartialValueWhenInitialized.update_env_of_not_pathConflicts
      henvSlot hnotWriteSource hnotWriteUpdated hrootVarsFresh hrootValidFinal
  have hpathCons : ∃ tail, path = () :: tail := by
    cases path with
    | nil => exact False.elim (hpathNonempty rfl)
    | cons head tail =>
  ...
  · intro y otherEnvSlot hyx henvY
    ...
    have hvalidOldFinalEnv :
        ValidPartialValueWhenInitialized
          ...
          oldValue otherEnvSlot.ty :=
      ValidPartialValueWhenInitialized.update_env_of_not_pathConflicts
        henvSlot hnotWriteSource hnotWriteUpdated hvarsOtherFresh
        hvalidOldStoreUpdated
...
theorem stored_var_reachesWhenInitialized_ne_owner_spine_leaf_of_noWrite
  ...
  · have hnotProtected : ¬ ProtectedByBase store x location := by
      ...
      rcases borrowDependencyWhenInitialized_protected_writeProhibited_or_mem_vars
          ...
          hprotected with hwp | hmem
      · exact hnotWriteSource hwp
...
theorem term_value_reachesWhenInitialized_ne_owner_spine_leaf_of_noWrite
  ...
  · have hnotProtected : ¬ ProtectedByBase store x location := by
      ...
      rcases borrowDependencyWhenInitialized_protected_writeProhibited_or_mem_vars
          ...
          (ValidRuntimeState.validStore hvalidRuntimeValue)
          (ValidRuntimeState.storeOwnerTargetsHeap hvalidRuntimeValue)
          hborrows hdependency hprotected with hwp | hmem
      · exact hnotWriteSource hwp
      · exact hnotWriteObserver (hvarsObserver x hmem)
    exact hnotProtected (by simpa [hlocation] using hleafProtected)
