...
theorem preservation_bounded
  ...
  case assign =>
    ...
    have hruntimeSafeInner : RuntimeFrame.RuntimeSafeAbstraction midStore _env₂ :=
      RuntimeFrame.RuntimeSafeAbstraction.of_safeEvidence_and_selectedSafe
        (RuntimeFrame.SafeAbstractionEvidence.of_safe hsafeInner)
        (RuntimeFrame.RuntimeSelectedBorrowSafe.of_borrowSafeEnv
          hborrowSafeInner)
...
This is the same recursive preservation skeleton as `preservation_bounded`, but
the induction hypotheses carry only the paper-facing safe abstraction
`store ∼ₛ env`.  The assignment case uses
`preservation_assign_step_terminal_borrowSafeFree_of_wellFormed`, so the only
remaining non-mechanical relaxed-preservation graph obligation is the named
assignment-through-selected-borrow theorem above.
-/
theorem preservation_bounded_borrowSafeFree
    (fuel : Nat) {store finalStore : ProgramStore} {env₁ env₂ : Env}
    {typing : StoreTyping} {lifetime : Lifetime} {term : Term}
    {ty : Ty} {finalValue : Value} :
    term.size ≤ fuel →
...
which is false for the relaxed joined approximation.  The checked relaxed
replacement currently available without any borrow-safety assumption is the
safe-only `T-If` case in `Lemma_4_11_RelaxedPreservation`.
-/
